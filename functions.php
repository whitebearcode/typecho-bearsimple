    ...
    foreach ($emo as $v){
        if($v['type'] == 'image'){
            foreach ($v['container'] as $vv){
                ...
                elseif($type == 'reply'){
                $comment = str_replace($vv['data'], '<img width="20" height="20" src='.$dir.'/assets/owo/'.$vv['icon'] .''.'>', $comment);
                }
                else{
                $comment = str_replace($vv['data'], '<img width="30" height="30" src='.$dir.'/assets/owo/'.$vv['icon'] .''.'>', $comment);                    
                }
...
<script src="<?php Helper::options()->themeUrl('assets/manage/bearui.min.js?v='.themeVersion()) ?>"></script>
 ...
 icon"></i> 模块管理</li>
  ...
  <ul class="list">
      <li>您当前使用的主题版本仅适配兼容于Typecho V1.2 Beta及以上版本</li>
...
<div class="ui labeled button" tabindex="0">
  ...
  <a class="ui basic black left pointing label" href="https://github.com/whitebearcode/typecho-bearsimple/tree/master2">
    V<?php echo themeVersion(); ?>/V<div id="version"></div>
        </a>
</div>
...
<center><a class="ui button" style="margin-top:15px;margin-bottom:-15px;" href="<?php $security->index('/action/plugins-edit?activate=BsCore'); ?>">检测到核心插件未开启!请先戳这里开启并初始化Bearsimple核心插件</a></center>
  <?php endif; ?>
...
$Article_forma = new Typecho_Widget_Helper_Form_Element_Select('Article_forma', array('1' => '简洁图文',  '2' => '纯文字','3' => '图底文字','4' => '纯文字2','5' => '简洁图文2'), '2', '选择首页、分类等页面输出文章的展现样式', '若选择图文或图底文字，则当存在图片时展现图文样式，优先级：文章封面->附件首图->文章首图->自定义随机图片，无图片时不显示<br><font color="red">简洁图文样式请先在图片设置-图片剪裁设置中设置允许的域名，否则封面不显示</font>');
...
//4
 ...
 $CCFirewall = new Typecho_Widget_Helper_Form_Element_Select('CCFirewalls', array('1' => '关闭', '2' => '开启'), '1', '选择是否开启简易CC防护', '若选择开启,可能会存在误拦截!');
...
$editor = new Typecho_Widget_Helper_Form_Element_Select('editor', array('1' => '否',  '2' => '是'), '1', '是否开启编辑器优化', '若选择开启,则将替换官方传统编辑器');
...
HTML;
    ...
    $Diy = new Typecho_Widget_Helper_Form_Element_Select('Diy', array('1' => '开启DIY模式',  '2' => '关闭DIY模式'), '2', '是否开启DIY模式', '开启后将显示DIY模式面板');
