    ...
    <div class="bearui_config">
        <div>
            <div class="bearui_config__aside">
                <div class="logo"><div class="ui blue big label">BearSimple V1.32</div></div>
                <ul class="tabs">
...
$AdControl3Url->setAttribute('class', 'bearui_content bearui_module');
    ...
    $Like = new Typecho_Widget_Helper_Form_Element_Select('Like', array('1' => '是',  '2' => '否'), '2', ' <i class="compress icon"></i> 是否开启文章点赞', '若选择开启,则文章内将显示点赞按钮');
    $Like->setAttribute('class', 'bearui_content bearui_article');
    $form->addInput($Like->multiMode());
    if($options->Like == '1'){

        $Like_num = new Typecho_Widget_Helper_Form_Element_Text(
          'Like_num', NULL, '',
          '每日点赞次数', '请输入单人一天的点赞次数');
$Like_num->setAttribute('class', 'bearui_content bearui_article');
        $form->addInput($Like_num);
    }
    //顶部设置
