...
$Article_forma = new Typecho_Widget_Helper_Form_Element_Select('Article_forma', array('1' => '简洁图文',  '2' => '纯文字','3' => '图底文字','4' => '纯文字2'), '2', '选择首页、分类等页面输出文章的展现样式', '若选择图文或图底文字，则当存在图片时展现图文样式，优先级：文章封面->附件首图->文章首图->自定义随机图片，无图片时不显示;');
...
$vfx = new Typecho_Widget_Helper_Form_Element_Select('global_transparent', array('0' => '关闭',  '1' => '开启'), '0', '全局增加透明效果', '开启后全局将显示透明效果，非常的通透，有透明癖好的可以开启，建议在有设置网站背景的情况下开启，无背景的情况下开启和没效果差不多');
    $vfx->setAttribute('class', 'bearui_content bearui_global');
    $form->addInput($vfx->multiMode());  
...
require_once('core/upgrade.php');
    if (Helper::options()->SiteMap && Helper::options()->SiteMap !== 'close') {
        ...
        if(!file_exists($file)){
        if(false!==fopen($file,'w+')){
            $header_info=get_headers(Helper::options()->siteUrl.'/index.php/sitemap.xml');
            file_put_contents($file,$header_info[0]);
             }
        }
    }
//短代码
...
<div class="ui huge list">
  <div class="item">
    <div class="header" style="margin-bottom:5px">1、登录、回复可见</div>
    {bs-hide}隐藏内容{/bs-hide}或者[bs-hide]隐藏内容[/bs-hide]
  </div>
  <div class="item">
  <div class="header" style="margin-bottom:5px">2、Todolist</div>
    已完成：{bs-todo type=true}已完成的内容{/bs-todo}<br>
    未完成：{bs-todo type=false}未完成的内容{/bs-todo}
  </div>
  <div class="item">
  <div class="header" style="margin-bottom:5px">3、代码高亮</div>
    ```语言名称<br>
    代码内容<br>
    ```
  </div>
