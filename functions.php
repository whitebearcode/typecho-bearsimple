...
function reEmo($comment,$type){
    ...
    foreach ($emo as $v){
        if($v['category'] !== ''){
                if($type == 'comment'){
                $comment = str_replace($v['data'], '<img width="30" height="30" src='.$dir.'/assets/owo/'.$v['icon'] .''.'>', $comment);
                }
                elseif($type == 'reply'){
                $comment = str_replace($v['data'], '<img width="20" height="20" src='.$dir.'/assets/owo/'.$v['icon'] .''.'>', $comment);
                }
                else{
                $comment = str_replace($v['data'], '<img width="30" height="30" src='.$dir.'/assets/owo/'.$v['icon'] .''.'>', $comment);                    
                }
...
function reEmoPost($post){
    ...
    foreach ($emo as $v){
        if($v['category'] !== ''){
                ...
                $post = str_replace($v['data'], '<img style="display:inline-block;margin: 0;padding: 0;width:30px;height:30px;vertical-align: middle;" class="emoji" src="'.$dir.'/assets/owo/'.$v['icon'] .'"'.'>', $post);
        }
    }
    return $post;
}
...
if( class_exists( 'CSF' ) ) {
  ...
  CSF::createSection( $prefix, array(
    ...
    'fields'      => array(
        ...
        array(
            'type'    => 'content',
            'content' => '1、使用本主题尽量少安装插件，否则可能因存在冲突导致各种各样的问题<br>2、主题用户交流QQ群:561848356<br>3、主题讨论微社区:<a href="https://support.qq.com/products/314782?">戳这里访问</a><br>不懂的问题或本主题存在的问题可加群或在微社区中进行反馈.          <center>  <br>
...
<script src="https://lf6-cdn-tos.bytecdntp.com/cdn/expire-1-M/jquery/3.6.0/jquery.slim.min.js" type="application/javascript"></script>
<script>
$(function() {
...
$("#versiontips").html(\'<div class="csf-submessage csf-submessage-warning">检测到有新版本可以更新，请及时完成更新!</div>\');
}
break;
case "error":
...
array(
                'type'    => 'notice',
            'style'   => 'info',
                'content' => '<ul style="margin:0 auto;"><li>BearSimple主题版本：V2.0.0 Release[20221129]</li><li>PHP版本：'.PHP_VERSION.'</li><li>网站服务器：'.$_SERVER['SERVER_SOFTWARE'].'</li><li>数据库软件：'.$db->getAdapterName().'[Version：'.$db->getVersion().']</li><li>Typecho版本：'.$Tyoptions->version.'</li><li>User Agent信息：'.$_SERVER['HTTP_USER_AGENT'].'</li></ul>',
        ),
  ...
  CSF::createSection( $prefix, array(
    ...
    'fields' => array(
      ...
      array(
            ...
            'default' => 'cravatar',
            'after' => 'Cravatar服务：Cravatar适合在国内使用，优先级:Cravatar头像->Gravatar头像->QQ头像<br>Gravatar服务：在国内官方源可能访问不了头像，支持了多个镜像源并支持您填写自定义镜像源<br>自定义头像：您可以通过自定义头像在前台为每个用户显示出随机头像，在自定义头像栏为空时默认为Cravatar',
        ),  
        array(
        'id'    => 'DiyAvatar',
        'type'  => 'textarea',
      ...
      array(
            ...
            'options'     => array('1' => 'Gravatar官方源','2' => 'LOLI.TOP*Gravatar镜像源', '3' => 'V2EX*Gravatar镜像源','4' => 'LOLI.NET*Gravatar镜像源','5' => '极客族*Gravatar镜像源','6' => '七牛*Gravatar镜像源','6-2' => 'Zeruns*Gravatar镜像源','7' => '自定义Gravatar镜像源'),
            'default' => '5',
    ...
    'fields' => array(
      array(
        ...
        array(
            ...
            'type'    => 'checkbox',
            'title'   => '编辑器自动保存方式选择',
            'subtitle' => '自动保存支持两种方式，两种方式可同时开启不受影响，全都不选代表不开启自动保存<br>第一种即为浏览器保存，文章在还没点击提交的情况下均保存在浏览器中，点击其他文章，先前文章的内容仍会直接覆盖到当前文章。第二种是保存为草稿，编辑器在每隔十五秒会将文章保存为草稿。',
            'inline'  => true,
            'options' => array(
                '1' => '浏览器自动保存',
                '11' => '自动保存为草稿',
...
仓库显示该Github账号下的所有项目，当然，您要想前台能看到的话必须新建一个独立页面，并将自定义模板选择我的Github仓库',
  ...
  CSF::createSection( $prefix, array(
    ...
    'fields' => array(
        ...
        array(
            'id'       => 'CommentNotify__AuthorTemplate',
            'type'     => 'code_editor',
            'title'    => '博主接收邮件模板',
            'subtitle' => '您可以根据实际需要修改模板内容，支持HTML',
            'default'=> file_get_contents(Helper::options()->themeUrl.'/modules/MailServices/owner.html'),
            'dependency' => array( 'CommentNotify__Open', '==', 'true' ),
        ),
        array(
            'id'       => 'CommentNotify__ReaderTemplate',
            'type'     => 'code_editor',
            'title'    => '访客接收邮件模板',
            'subtitle' => '您可以根据实际需要修改模板内容，支持HTML',
            'default'=> file_get_contents(Helper::options()->themeUrl.'/modules/MailServices/guest.html'),
            'dependency' => array( 'CommentNotify__Open', '==', 'true' ),
        ),
  ...
  CSF::createSection( $prefix, array(
    ...
    'fields' => array(
      array(
            ...
            'title'       => '文章微海报',
            'default' => false,
            'subtitle'=>'开启后前台文章页面会显示微海报生成按钮，在每篇文章的编辑页面都会有单独的海报生成开关选择，当本项开启而文章编辑页面选择不开启时则不开启，当文章编辑页面海报生成选择开启而本项不开启时则不开启，需要两项同时开启才算开启状态！'
        ),
        array(
            'id'       => 'Poster__LogoUrl',
            'type'    => 'upload',
...
if(json.version == nowversion){
$("#versiontipss").html(\'<div class="csf-submessage csf-submessage-success">当前版本为最新版本，无需更新~</div>\');
...
$.ajax({
                        ...
                        success: function(json) {
                           ...
                           $("#pre-message").html("升级包大小："+json.filesize+"，预计需要三分钟，请耐心等待~");
...
<?php
    ...
    $params = [
        'args'=> [
            'framework_title' => 'Bearsimple V2.0.0',
