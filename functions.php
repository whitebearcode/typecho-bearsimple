...
//4
 ...
 //特效设置
    ...
    $Compress = new Typecho_Widget_Helper_Form_Element_Select('Compress', array('1' => '开启HTML压缩',  '2' => '关闭HTML压缩'), '2', '是否开启HTML压缩', '开启后能够加快网站访问');
...
//顶部设置
  //页面输出方式自定义选择
    ...
    $waterMarkName = new Typecho_Widget_Helper_Form_Element_Text('waterMarkImg', NULL, '', _t('水印文字'),'填写水印文字,不填默认显示网站文字LOGO');
        $waterMarkName->setAttribute('class', 'bearui_content bearui_images');
        $form->addInput($waterMarkName);
    
    $waterMarkAxisX = new Typecho_Widget_Helper_Form_Element_Text('waterMarkAxisX', NULL, '', _t('水印文字所在位置X轴'),'填写水印文字所在位置X轴，九宫格，填写数字即可，不填则默认180');
        $waterMarkAxisX->setAttribute('class', 'bearui_content bearui_images');
        $form->addInput($waterMarkAxisX);
        
    $waterMarkAxisY = new Typecho_Widget_Helper_Form_Element_Text('waterMarkAxisY', NULL, '', _t('水印文字所在位置Y轴'),'填写水印文字所在位置Y轴，九宫格，填写数字即可，不填则默认280');
        $waterMarkAxisY->setAttribute('class', 'bearui_content bearui_images');
        $form->addInput($waterMarkAxisY);
        
    $waterMarkSize = new Typecho_Widget_Helper_Form_Element_Text('waterMarkSize', NULL, '', _t('水印文字大小'),'填写水印文字大小，单位为px，填写数字即可，不填则默认15px');
        $waterMarkSize->setAttribute('class', 'bearui_content bearui_images');
        $form->addInput($waterMarkSize);
        
     $waterMarkColor = new Typecho_Widget_Helper_Form_Element_Text('waterMarkColor', NULL, '', _t('水印文字颜色'),'填写水印文字颜色，如#000000，默认#000000');
        $waterMarkColor->setAttribute('class', 'bearui_content bearui_images');
        $form->addInput($waterMarkColor);
...
', '根据研究表明机器人发表的内容可能含有评论文章的标题');
    ...
    //海报设置
    $Poster = new Typecho_Widget_Helper_Form_Element_Select('Poster', array('1' => '是',  '2' => '否'), '2', '是否开启文章微海报功能', '开启后前台文章页面会显示微海报生成按钮，考虑到服务器配置因素，在每篇文章的编辑页面都会有单独的海报生成开关选择，当本项开启而文章编辑页面选择不开启时则不开启，当文章编辑页面海报生成选择开启而本项不开启时则不开启，需要两项同时开启才算开启状态！<br><a style="color:red">请注意，本功能略有些消耗服务器资源，且需支持GD库，慎重开启！');
    $Poster->setAttribute('class', 'bearui_content bearui_poster');
    $form->addInput($Poster->multiMode());
    
    $Poster_Webname = new Typecho_Widget_Helper_Form_Element_Text('Poster_Webname',null,'', '博客名称', '将显示在海报中的网站名称');
    $Poster_Webname->setAttribute('class', 'bearui_content bearui_poster');
    $form->addInput($Poster_Webname);
    
    $Poster_WebDec = new Typecho_Widget_Helper_Form_Element_Text('Poster_WebDec',null,'', '博客介绍', '将显示在海报中的网站介绍，务必言简意赅');
    $Poster_WebDec->setAttribute('class', 'bearui_content bearui_poster');
    $form->addInput($Poster_WebDec);
    
    $Poster_Bloger = new Typecho_Widget_Helper_Form_Element_Text('Poster_Bloger',null,'', '博主昵称', '将显示在海报中的文章编辑处');
    $Poster_Bloger->setAttribute('class', 'bearui_content bearui_poster');
    $form->addInput($Poster_Bloger);
    
    $Poster_QQ = new Typecho_Widget_Helper_Form_Element_Text('Poster_QQ',null,'', '您的QQ', '用于海报中的头像生成');
    $Poster_QQ->setAttribute('class', 'bearui_content bearui_poster');
    $form->addInput($Poster_QQ);
    
    $Poster_QQsetting = new Typecho_Widget_Helper_Form_Element_Select(
            'Poster_QQsetting' ,
            array(
                'close' => _t('无操作'),
                'type1' => _t('防举报'),
        ...
        $Poster_QQsetting->setAttribute('class', 'bearui_content bearui_poster');
        $form->addInput($Poster_QQsetting);
