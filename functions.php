...
<script src="<?php Helper::options()->themeUrl('assets/manage/bearui.min.js?v='.themeVersion()) ?>"></script>
	...
	<link href="https://lib.baomitu.com/semantic-ui/2.4.1/semantic.min.css" rel="stylesheet">

	<link href="https://lib.baomitu.com/toastr.js/2.1.4/toastr.min.css" rel="stylesheet">
	<script src="https://lib.baomitu.com/toastr.js/2.1.4/toastr.min.js"></script>
	<script>
	$(document).ready(function(){
	    ...
	    if(options3 == '1' || options3 == '5'){
          ...
          $("#typecho-option-item-music-121").hide();   
...
//顶部设置
  //增加登录
    ...
    //模块管理新增编辑器保存
    $EditorsAutosave = new Typecho_Widget_Helper_Form_Element_Select('EditorsAutosave', array('1' => '是',  '2' => '否'), '1', '编辑器是否开启自动保存', '若开启自动保存，考虑到一些情况，文章在还没点击提交的情况下均保存在浏览器中，点击其他文章，先前文章的内容仍会直接覆盖到当前文章。');
    $EditorsAutosave->setAttribute('class', 'bearui_content bearui_modules');
    $form->addInput($EditorsAutosave->multiMode());
...
<div class="ui huge list">
  ...
  <div class="item">
  <div class="header" style="margin-bottom:3px">6、音频</div>
   {bs-audio}音频地址{/bs-audio}<br>
   音频地址必须直链
  </div>
  <div class="item">
  <div class="header" style="margin-bottom:3px">7、手风琴</div>
  线性手风琴<br>
  ...
  <div class="item">
  <div class="header" style="margin-bottom:3px">8、Github仓库</div>
  {bs-card type=github projectname=Github仓库名称 projectdec=Github仓库描述 projecturl=Github仓库地址,不带http(s)://}{/bs-card}<br>
  <font color=red>Github仓库地址一定不要带http(s)://</font>
  </div>
</div></div>
