...
?>
        ...
        <li id="<?php $this->theId(); ?>" <?php echo $hidden;?>><div class="friendcircle-main-level"><div class="friendcircle-avatar"><img <?php echo lazyload('auto');?>src="<?php echo $avatar; ?>"></div><div class="friendcircle-box"><div class="friendcircle-head"><h3 class="friendcircle-name by-author"><?php CommentAuthor($this); ?></h3> </div>
        <div class="friendcircle-content" style="background-color:#fff!important"><?php echo reEmo($this->content,"circle"); ?>
        <?php if(!empty(self::circle_getInfo($this)['resources'])):?>
        <div class="friendcircle-pictures">
        <?php foreach(json_decode(self::circle_getInfo($this)['resources'],true) as $resource):?>
        <img data-fancybox="image" <?php echo $lazyload;?>src="<?php echo $resource;?>"> 


        <?php endforeach;?>
        </div>  
        <?php endif;?>
        </div><?php if(!empty(self::circle_getInfo($this)['location'])):?><a class="friendcircle-map" href="https://map.baidu.com/search/<?php echo self::circle_getInfo($this)['location'];?>" target="_blank"><?php echo self::circle_getInfo($this)['location'];?></a><?php endif;?><span class="friendcircle-time">
