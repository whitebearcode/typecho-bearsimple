...
function thumb($obj) {
	...
	if($cover){
		...
		$thumb = $thumbs[rand(0,count($thumbs)-1)].'?'.mt_rand(1,9999);
	}
	else{
	    $thumb = 'null';
	}
...
function thumb2($obj) {
	...
	if($cover){
		...
		$thumb = $thumbs[rand(0,count($thumbs)-1)].'?'.mt_rand(1,1000);
	}
	else{
	    $thumb = 'null';
	}
...
function lazyload($haveClass = 0,$url = ''){
    ...
    elseif($haveClass == 'auto' && !empty($url)){
        return ' class="lazyload bs-blur" src="'.$url.'" data-';
    }
    elseif($haveClass == 'auto' && empty($url)){
        return ' class="lazyload bs-blur" ';
    }
...
function lastComments(){
    ...
    $search_Crosspage = $db->fetchAll($db->select('cid')->from('table.contents')
        ...
        if($search_Crosspage[0]['cid']){
            $result = $db->select('text','author','created','cid','coid','mail')->from('table.comments');
    ...
    echo '<div class="label" style="margin:auto">
      <img '.lazyload('auto',imgravatarq($comment['mail'])).'src="'.imgravatarq($comment['mail']).'">
    </div>';
        }
        echo '
    <div class="content">
      ...
      echo '<div class="ui basic label">'.excerpt_content(bearcutword(reEmo($comment['text'],'reply'))).'</div>';    
      }
      }
      else{
         echo '<div class="ui basic label">'.excerpt_content(bearcutword(reEmo($comment['text'],'reply'))).'</div>';     
...
class Widget_Post_hot extends Widget_Abstract_Contents
{
    ...
    $col = 'Cate_Encrypt_Id';
        ...
        if(empty($mid) || ($user->hasLogin() && $user->pass('administrator'))){
