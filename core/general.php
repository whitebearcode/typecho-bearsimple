...
$md = new Markdown();
$targetSummary = excerpt($md::convert($val['text']), 30);
...
echo '<div class="bs-morepost-slide"><div class="bs-morepost-content">
        <h3 class="post-title"><a href="'.$permalink.'">'.$post_title.'</a></h3>
         <ul class="post-bar">
                            ...
                            <a href="'.$permalink.'" class="ui icon button"><i class="book open icon"></i> 阅读</a>
                        </div>
                    </div>';
}
}
...
function tags($widget, $split = '', $default = NULL)
{
    ...
    if ($widget->tags) {
        ...
        $result[] .= '<a class="ui label" href="'.$tag['permalink'].'">#'.$tag['slug'].'</a>';
        }
 $result[] .= '</div>';
