 ...
 function excerpt_content($content)
    {
        ...
        if (strpos($content, '[bsfra') !== false) {
            $pattern = get_shortcode_regex2(array('bsfra'));
            $content = preg_replace("/$pattern/", '', $content);
        }
...
function parseGithubCallback($matches)
    {
        ...
        return <<<EOF
        <div class="ui relaxed divided list" style="margin:auto"><div class="item"><i class="big github middle aligned icon"></i><div class="content"><a class="header" href="{$result['html_url']}" title="{$result['description']}">{$result['full_name']}</a><div class="description">{$star} {$date}</div></div></div></div>
EOF;
    }
...
function parseMusicCallback($matches)
{
    ...
    <script>
    fetch("https://api.i-meto.com/meting/api?server=netease&type=playlist&id={$matches[5]}").then(async response => {
...
function ShortCodePage($post,$title,$remember,$cid,$login,$articletype,$modetype='noreadmode'){
 ...
 $replacement = '
<img class="bearmark lazyload lazyload-style-'.$options['Lazyload_style'].'" '.lazyload(1).'src="$1" referrerPolicy="no-referrer"  alt="'.$title.'" title="点击放大图片">'.$alt; 
...
//Mermaid
 ...
 $replacement = '
  <a class="card">
    <div class="image">
      <img class="bearmark lazyload lazyload-style-'.$options['Lazyload_style'].'" '.lazyload(1).'src="$1" referrerPolicy="no-referrer"  alt="$2" title="点击放大图片">
    </div>
  </a>'; 
    $content = preg_replace($pattern, $replacement, $content);
...
function ShortCode($post,$t,$login,$articletype,$modetype='noreadmode'){
 ...
 $replacement = '
<img class="pure-img bearmark lazyload lazyload-style-'.$options['Lazyload_style'].'" '.lazyload(1).'src="$1" referrerPolicy="no-referrer" alt="$2" title="点击放大图片">'.$alt; 
    $content = preg_replace($pattern, $replacement, $content);
 ...
 $replacement = '
<img class="pure-img lazyload lazyload-style-'.$options['Lazyload_style'].'" '.lazyload(1).'src="$1" referrerPolicy="no-referrer"  alt="$2" title="$2">'.$alt; 
    $content = preg_replace($pattern, $replacement, $content);
    }
 ...
 $replacement = '
  <a class="card">
    <div class="image">
      <img '.lazyload(1).'src="$1" class="bearmark lazyload lazyload-style-'.$options['Lazyload_style'].'" referrerPolicy="no-referrer"  alt="$2" title="点击放大图片">
    </div>
  </a>'; 
    $content = preg_replace($pattern, $replacement, $content);
