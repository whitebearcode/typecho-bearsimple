...
<?php endif ; ?>
                ...
                <button type="submit" class="ui blue mini labeled submit icon  button" style="float:right"><i class="location arrow icon"></i><?php _e('发送'); ?></button>
...
<div class="bs-timeline-block" id="<?php $comments->theId(); ?>">
	...
	<img src="<?php echo imgravatarq($comments->mail); ?>" alt="Picture">		    
     	</div>
			<div class="bs-timeline-content">
			    <?php $comments->date('Y-m-d H:i'); ?>
			    <hr style="border: 0;padding-top: 1px;background: repeating-linear-gradient(to right, #a2a9b6 0px, #a2a9b6 4px, transparent 0px, transparent 10px);background: repeating-linear-gradient(to right, #a2a9b6 0 var(--dashed-filled, 4px), transparent 0 calc(var(--dashed-filled, 4px) + var(--dashed-open, 6px)));">
       	<p> <?php echo reEmo($comments->content,'comment'); ?>	</p>
    	</div>
		</div>
<?php } ?>
