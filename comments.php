...
<?php if($this->options->Emoji == '1') :?>
  ...
  <?php endif; ?>
<h3 class="ui dividing header" id="<?php if($this->allow('comment') && $this->options->CommentClose == "1"): ?>comments<?php else:?>comms<?php endif; ?>"><i class="comments outline icon" data-content="若您进行评论则表示您同意本站使用Cookie来记录您所填写的个人信息以便您在后面能够更便捷的进行评论。" data-variation="inverted"></i>评论区 </h3>
<?php if($this->allow('comment') && $this->options->CommentClose == "1"): ?>
<?php
...
<a class="ui big circular image avatar">
     ...
     <div class="content">
      <a class="author" style="margin-left:10px"><a><?php $comments->author(); ?></a></a><?php 
        if ($comments->authorId == $comments->ownerId) {
  ...
  <i class="user icon"></i>读者
        ...
        ?>
      <?php if(Helper::options()->Comment_useragent == "2"): ?> <?php BearsimpleUserAgent::render($comments->agent,'comment'); ?><?php endif; ?>

      <div class="metadata">
        <span class="date"><?php $comments->date('Y-m-d H:i'); ?></span>
      </div>
      
      <div class="text" style="word-break:break-all;">
          <div class="ui left pointing basic label">
              <?php if(Helper::options()->CommentCoid == '1') :?><b><?php echo getPermalinkFromCoid($comments->parent); ?></b><?php endif; ?><?php getCommentHF($comments->coid); ?>
        <?php echo reEmo($comments->content,'comment'); ?>
      </div></div>
      ...
      <div class="actions">
        <a class="reply">
           ...
           <i id="commentlike" class="like thumbs up red link icon" data-coid="<?php echo $comments->coid; ?>"></i><font color=gray class="agreenumcomment<?php echo $comments->coid; ?>"><?php echo $agree['agree']; ?></font>
           <?php endif; ?>
      </div>
    </div>
...
<div id="<?php if($this->allow('comment') && $this->options->CommentClose == "1"): ?>comments<?php else:?>comms<?php endif; ?>" class="comment">
  ...
  <i class="reply icon"></i>取消回复
    	...
    	<form method="post" action="<?php $this->commentUrl() ?>" id="commentform" role="form" class="ui equal width form">
  ...
  当前身份:<?php $this->user->screenName(); ?>
    		  ...
    		  <a href="<?php $this->options->logoutUrl(); ?>" title="Logout"><i class="icon sign-out"></i></a><br><br>
            <?php else: ?>
<div class="field">
                <label for="author" class="required"><?php _e('昵称'); ?></label>
                <div class="ui icon input">
    			<input type="text" name="author" id="author" class="text" placeholder="请输入昵称" value="<?php $this->remember('author'); ?>" required />
        <i id="randname"  class="circular bolt link icon"></i>
</div>
    	  		</div>
                <div class="field">
                <label for="mail"<?php if ($this->options->commentsRequireMail): ?> class="required"<?php endif; ?>><?php _e('邮箱'); ?></label>
    			<input class="text" type="email" name="mail" id="mail"  placeholder="请输入邮箱地址" value="<?php $this->remember('mail'); ?>" required />
    			</div>
    		<div class="field">
...
<?php if($this->options->VerifyChoose == '4') :?>
                 <?php BsCore_Plugin::outputSimpleCommentCaptchaField($this); ?>
             	<p>
                <button type="submit" class="ui primary button"><i class="icon edit"></i> <?php _e('提交评论'); ?></button>
            </p>
             <?php endif ; ?>
    ...
    <div class="vaptcha-init-main">
        <div class="vaptcha-init-loading">
            ...
            <?php if($this->options->VerifyChoose == '22') :?>
             ...
             <span class="ui button" id="qverify" onclick="Verify();"><i class="user secret icon"></i><?php if(empty($this->options->Verify22_Buttontext)){echo '人机验证';}else{
			echo $this->options->Verify22_Buttontext();}
			  ?></span>
	<div id="QVerify" style="display:none;"></div>
    		<p>
...
<script src="<?php AssetsDir();?>assets/js/bearsimple-comments.js"></script>
<?php else:?>
<?php _e('<div class="ui big label">评论已关闭</div>'); ?></div></div>
<?php endif; ?>
