...
<div class="pure-g" id="layout">
 ...
 <?php \Widget\Metas\Tag\Admin::alloc()->to($ptags); ?>


<?php if($ptags->have()): ?>
<?php
$page = isset($_GET['page'])?$_GET['page']:"1";
$rePage = autoPage(getTags(),"30",$page);

foreach($rePage['ret'] as $key=>$val){
  echo '<a  href="'.$val['url'].'" class="ui left labeled button" style="margin:5px;text-align:center;"><div class="ui basic label">'.$val['name'].'</div><div class="ui icon button"><i class="tag icon"></i></div></a>';
}
echo $rePage['page'];
?>
<?php else: ?>
