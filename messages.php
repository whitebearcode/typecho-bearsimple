...
<?php if ($this->allow('comment') && Bsoptions('CommentClose') == true): ?>
        ...
        <div id="<?php $this->respondId(); ?>" class="respond">
            ...
            <h3 id="response"><?php _e('我要留言'); ?><?php if(Bsoptions('Comment_tips') == true || Bsoptions('Comment_tips') == ''):?><span class="bstooltip bstooltip--right bstooltips--right" data-bstooltip="<?php if(Bsoptions('Comment_tips_text') == ''):?>本博客使用Cookie技术保留您的个人信息以方便您能够在下一次快速留言，继续留言表示您已同意该条款<?php else:?><?php echo Bsoptions('Comment_tips_text'); ?><?php endif; ?>"><i class="question circle outline icon"></i></span><?php endif; ?></h3>
...
function getCommentInfo(){
    ...
    if (nick) nick.value = decodeURIComponent(getCookie('<?php echo Typecho_Cookie::getPrefix()?>__typecho_remember_author'));
    if (mail) mail.value = decodeURIComponent(getCookie('<?php echo Typecho_Cookie::getPrefix()?>__typecho_remember_mail'));
    if (url) url.value = decodeURIComponent(getCookie('<?php echo Typecho_Cookie::getPrefix()?>__typecho_remember_url'));
