 ...
 <div id="loading"  class="ui icon message" style="display:none">
  ...
  <div class="content">
    ...
    <p>请稍等一下哦～马上就出来了</p>
                ...
                <div class="finish_content ui three column doubling stackable masonry grid"></div>
<br>
            <!--分页链接-->
            <div class="pagelist">
            </div>
...
str += '<div class="column"><div class="ui fluid card"><div class="content" style="word-break:break-all;"><div class="ui top left attached label">' + list[i]['language'] + '</div><a class="header" href="' + list[i]['url'] + '"<?php if(Bsoptions('Link_blank') == true):?> target="_blank"<?php endif; ?>>' + list[i]['name'] + '</a><div class="meta">更新于' + list[i]['push'] + '</div><div class="description"><p>' + list[i]['dec'] + '</p></div></div><div class="extra content"><i class="star yellow icon"></i>' + list[i]['stars'] + '<g style="float:right;"><i class="fork green icon" title="Forks"></i>' + list[i]['forks'] + '</g></div></div></div>'
}
