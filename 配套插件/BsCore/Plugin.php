...
public static function EncryptCategory(){
   ...
   public static function CateFilter($thisx, $select){
       ...
       if ("Pdo_SQLite" === $adapter || "SQLite" === $adapter) {
        ...
        if(empty($mid) || ($user->hasLogin() && $user->pass('administrator'))) return $select;       //没有加密分类或用户已登录[管理员身份]，则直接返回
...
if(($obj->mail==$commentsMail && $obj->authorId==0)||(!empty($parentmail) && in_array($commentsMail,$parentmail))){
 ...
 if(!$_POST['type'] || $_POST['type'] !== 'cross'){ //判断POST过来的type字段是否为微语或为空
...
$postdata = array('secret' => $options['turnstile_secretkey'], 'response' => $_POST['cf-turnstile-response']);
			$options = array(
				'http' => array(
					'method' => 'POST',
    ...
    if ($response_data->success !== true) {
        \Typecho\Cookie::set('__typecho_remember_text', $comment['text']);
			throw new \Typecho\Widget\Exception("抱歉，您未通过人机验证，请重新验证",200);
    }
 }
