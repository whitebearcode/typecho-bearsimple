    ...
    \Typecho\Plugin::factory('Widget_Upload')->attachmentDataHandle = [__CLASS__, 'upyun_attachmentDataHandle'];
       ...
       //\Typecho\Plugin::factory('admin/header.php')->header_1081 = [__CLASS__, 'loginverify'];
        ...
        \Typecho\Plugin::factory('Widget_Comments_Edit')->finishComment = [__CLASS__,'parseComment'];
         ...
         if(get_option('bearsimple')['Tinypng_open'] == true && get_option('bearsimple')['bucket_choose'] !== '2'&& get_option('bearsimple')['bucket_choose'] !== '3'&& get_option('bearsimple')['bucket_choose'] !== '4'&& get_option('bearsimple')['bucket_choose'] !== '5'){
...
if ($opts['bucket_other'] !== '') {
  ...
  /**
    ...
    $file['tmp_name'] = $path;
        ...
        $upload_root = \Typecho\Common::url(defined('__TYPECHO_UPLOAD_DIR__') ? __TYPECHO_UPLOAD_DIR__ : self::UPLOAD_DIR,
                            defined('__TYPECHO_UPLOAD_ROOT_DIR__') ? __TYPECHO_UPLOAD_ROOT_DIR__ : __TYPECHO_ROOT_DIR__);
        
        $options = get_option('bearsimple');
        $user_dir     = $options['ali_bucket_path'];
       ...
       $oss_client = self::ossInit();
        ...
        $save_on_server = $options['ali_bucket_localsave'];

        $file_origin_name = self::getSafeaName($file['name']);
        $relative_path = date('Y/m/d/');
        
        $remote_file_name = $user_dir . $relative_path . $file_origin_name;

        if ($save_on_server === "1" && !\Typecho\Common::isAppEngine()) {
    ...
    public static function ali_modifyHandle($content, $file)
    {
        ...
        $options = get_option('bearsimple');
        $userDir     = $options['ali_bucket_path'];
        $bucket_name = $options['ali_bucket_name'];
         $end_point   = 'http://' . (($options['ali_bucket_region'] === "other") ?
                $options['ali_bucket_other_region'] :
                $options['ali_bucket_region'] . $options['ali_bucket_other']);
        ...
        $ifLoaclSave = $options['ali_bucket_localsave'];
        $upload_root = \Typecho\Common::url(defined('__TYPECHO_UPLOAD_DIR__') ? __TYPECHO_UPLOAD_DIR__ : self::UPLOAD_DIR,
                    defined('__TYPECHO_UPLOAD_ROOT_DIR__') ? __TYPECHO_UPLOAD_ROOT_DIR__ : __TYPECHO_ROOT_DIR__);
        ...
        if (200 != $ali_response['info']['http_code']) {
            ...
            if ($ifLoaclSave === "1" && !\Typecho\Common::isAppEngine()) {
    ...
    public static function ali_attachmentHandle(array $content) {
        ...
        $cdnUrl     = $options['ali_bucket_domain'];
        $userDir    = $options['ali_bucket_path'];
        if (preg_match('/\.(jpg|jepg|png|ico|bmp|gif|tiff|webp)/i', $content['attachment']->path)) {
  ...
  public static function qiniu_uploadHandle($file)
    ...
    $file['tmp_name'] = $path;
        ...
        $options = Helper::options();
        $date = new \Typecho\Date($options->gmtTime);
        $fileDir = self::qiniu_getUploadDir()  . $date->year . '/' . $date->month;
        $fileName = sprintf('%u', crc32(uniqid())) . '.' . $ext;
        $path = $fileDir . '/' . $fileName;
        $uploadfile = self::getUploadFile($file);
        if (!isset($uploadfile)) {
...
return self::qiniu_getAuth()->uploadToken($options['qiniu_bucket_name'], $key);
  ...
  public static function upyun_uploadHandle($file)
    ...
    $file['tmp_name'] = $path;
        ...
        $fileDir = self::upyun_getUploadDir()  . $date->year . '/' . $date->month;
 ...
 //代码高亮
    ...
    if(bsOptions::getInstance()::get_option('bearsimple' )['Codehightlight'] == true){
       ...
       $style = bsOptions::getInstance()::get_option( 'bearsimple' )['code_style'];
        ...
        $playerUrl = $dir.'assets/vendors/player/player.min.css?v=2.2.4';
  ...
  <script src="//cdn.staticfile.org/alpinejs/3.10.5/cdn.min.js"></script>
    ...
    <script src="{$dir}assets/vendors/player/hls.min.js" type="text/javascript"></script>
    <script src="{$dir}assets/vendors/player/hls.min.js" type="text/javascript"></script>
    <script src="{$dir}assets/vendors/player/player.min.js?v=2.3.0" type="text/javascript"></script>
