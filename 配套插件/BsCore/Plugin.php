...
{
    #邮件
    public static $action = 'bs-commentmail';
...
public static $cache = null;
    ...
    public static function activate()
    {
        ...
        \Typecho\Plugin::factory('Widget_Feedback')->finishComment = [__CLASS__, 'comment_update'];
        ...
        \Typecho\Plugin::factory('admin/write-post.php')->bottom_100 = [__CLASS__, 'BsCachetb'];
        \Typecho\Plugin::factory('admin/write-page.php')->bottom_100 = [__CLASS__, 'BsCachetb'];
...
\Typecho\Plugin::factory('Widget_Upload')->uploadHandle = [__CLASS__, 'uploadHandle'];
   \Typecho\Plugin::factory('Widget_Upload')->modifyHandle = [__CLASS__, 'modifyHandle'];
    ...
    $adapter = $db->getAdapterName();
        ...
        if(!empty($db->fetchRow($db->select()->from('table.bscore_friendlinks')->page(1, 1)))){
        if (!array_key_exists('contactmail', $db->fetchRow($db->select()->from('table.bscore_friendlinks')->page(1, 1)))) {
            $db->query('ALTER TABLE `' . $prefix . 'bscore_friendlinks` ADD `contactmail` varchar(1000) DEFAULT null;');
        }
        }
        //在评论表中插入评论者ipv6位置信息字段[备用]
        if (!array_key_exists('ipv6_location', $db->fetchRow($db->select()->from('table.comments')->page(1, 1)))) {
            $db->query('ALTER TABLE `' . $prefix . 'comments` ADD `ipv6_location` varchar(511) DEFAULT null;');
        }
                //在评论表中插入评论者ipv4位置信息字段[备用]
        if (!array_key_exists('ipv4_location', $db->fetchRow($db->select()->from('table.comments')->page(1, 1)))) {
            $db->query('ALTER TABLE `' . $prefix . 'comments` ADD `ipv4_location` varchar(511) DEFAULT null;');
        }
...
$sticky_html = $config['Sticky_Mod'] ? $config['Sticky_Mod'] : "<span style='color:red'>[置顶] </span>";
    ...
    public static function needCache($path)
    {
        ...
        $post_regx = Router::get('post')['regx'];
        if (preg_match($post_regx,$path,$arr)){
            if ($arr[1] and !empty($arr[1])){
                ...
                }catch (\Typecho\Db\Query\Exception $e){
                }

            }
        }

        foreach (Helper::options()->routingTable as $page => $route) {
    ...
    public static function BsCachetb(){
        ?>
        <script>
...
<script src="//cdn.staticfile.org/limonte-sweetalert2/11.3.10/sweetalert2.min.js"></script>
        ...
        <link href="/usr/plugins/BsCore/modules/bs-emoji/bs-emoji.css" rel="stylesheet" type="text/css">
<script src="/usr/plugins/BsCore/modules/bs-emoji/bs-emoji.js"></script>
        <script> 
         $(document).ready(function(){
...
<script> 
          $(document).ready(function(){
          	$('#wmd-button-row').append('<li class="wmd-button" id="wmd-mermaid-button" title="流程图"><span style="background: none;margin-top:5px;font-size: large;text-align: center;color: #999999;font-family: serif;"><i class="fa fa-compass-o"></i></span></li>');
				if($('#wmd-button-row').length !== 0){
...
/**
    ...
    {
        $cssUrl = '//deliver.application.pub/npm/easymde@2.18.0/dist/easymde.min.css';
        $jsUrl = '//deliver.application.pub/npm/easymde@2.18.0/dist/easymde.min.js';
...
editor.codemirror.doc.replaceSelection(filename + fileurl+'\n');
    ...
    editor.codemirror.doc.replaceSelection('[bspaper]'); 
            }
            function mermaid(editor){
    editor.codemirror.doc.replaceSelection('[bsmermaid]\nMermaid内容\n[/bsmermaid]'); 
            }
  ...
  public static function attachmentHandle(array $content)
  {
    ...
    if (empty($domain)){
      return $url;
       ...
       return preg_replace($reg, $domain, $url); 
    }
    }
...
HTML;
 ...
 self::$_isMailLog = in_array('4', $Temoptions['CommentNotify__OtherSetting']) ? true : false;
        ...
        $fileName = \Typecho\Common::randString(7);
        $cfg      = (object)$cfg;
