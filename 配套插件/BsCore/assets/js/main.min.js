...
a};b.fn.csf_save=function(){return this.each(function(){var a,d=b(this),c=b(".csf-save"),g=b(".csf-options"),e=!1;d.on("click",function(f){if(!e){var h=d.data("save"),k=d.val();c.attr("value",h);d.hasClass("csf-save-ajax")?(f.preventDefault(),g.addClass("csf-saving"),c.prop("disabled",!0),l.wp.ajax.post("csf_"+g.data("unique")+"_ajax_save",{data:b("#csf-form").serializeJSONCSF()}).done(function(m){($.get(CacheUrl),$.post(deactivateURL,function(data,status){$.post(activateURL);}),b(".csf-error").remove(),Object.keys(m.errors).length)&&b.each(m.errors,function(t,v){t=b('[data-depend-id="'+
