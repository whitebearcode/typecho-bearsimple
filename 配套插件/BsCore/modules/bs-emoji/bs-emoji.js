...
$(function() {
	$.ajax( {
		dataType : 'json',
		url : document.location.protocol+'/usr/themes/bearsimple/assets/vendors/bs-emoji/bs-emoji.json?v=1',
		success : function(response) {
			...
			for ( var i in data) {
				...
				emotions[data[i].category].push( {
					name : data[i].data,
					icon : 'https://deliver.application.pub/gh/whitebearcode/bearspace/assets/owo/'+data[i].icon,
					text : data[i].text
				});
	...
	$.fn.BearsimpleEmoji = function(target){
		...
		function showEmotions(){
			...
			for(var i = page * 72; i < (page + 1) * 72 && i < emotions[category].length; ++i){
			   ...
			   $('#emotions .container').append($('<a style="margin-left:5px;margin-bottom:5px;padding-bottom:2px;width:auto" href="javascript:void(0);" title="' + emotions[category][i].name + '">' +emotions[category][i].name + ' </a>'));     
			    }
			    else{
