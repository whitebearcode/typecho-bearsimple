 ...
 * @修改: WhiteBear <whitebear@msn.com>
 * @license BSD License
 */
class Parser
{
     ...
     * @var string
     */
    private $_commonWhiteList = 'div|h1|h2|h3|h4|h5|h6|table|blockquote|pre|dl|ol|kbd|b|i|strong|em|sup|sub|br|code|del|a|hr|small|center|p|img|li|ul|iframe|svg';
     ...
     * @var string
     */
    private $_blockHtmlTags = 'p|div|blockquote|pre|table|dl|ol|ul|address|form|fieldset|iframe|hr|legend|article|section|nav|aside|hgroup|header|footer|figcaption|svg|script|noscript';
 ...
 if ($lang == 'mermaid') {
        return $isEmpty ? '' :
            '<div' . (!empty($lang) ? " class=\"{$lang}\"" : '')
            . (!empty($rel) ? " rel=\"{$rel}\"" : '') . '>'
            . $str . '</div>';
   }
   else{
        return $isEmpty ? '' :
            '<pre><code' . (!empty($lang) ? " class=\"lang-{$lang}\"" : '')
            . (!empty($rel) ? " rel=\"{$rel}\"" : '') . '>'
            . $str . '</code></pre>';
   }
    }
