...
if ( ! class_exists( 'CSF_Field_map' ) ) {
  class CSF_Field_map extends CSF_Fields {
    ...
    public $cdn_url = 'https://lf26-cdn-tos.bytecdntp.com/cdn/expire-1-M/leaflet/';
...
//      }
        ...
        if ($enq_js){
            return enqueue_script_helper('csf-leaflet', esc_url( $this->cdn_url . $this->version .'/leaflet.min.js' )).enqueue_script_helper('jquery-ui-autocomplete');
        } else {
            return enqueue_style_helper('csf-leaflet', esc_url( $this->cdn_url . $this->version .'/leaflet.min.css'));
        }
