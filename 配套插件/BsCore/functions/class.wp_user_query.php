...
if (!class_exists('WP_User_Query')) {
    ...
    class WP_User_Query
    {
         ...
         * @type string[] $search_columns Array of column names to be searched. Accepts 'ID',
         *                                                'mail', 'url', 'name', 'display_name'.
         *                                                Default empty array.
...
//        do_action_ref_array( 'pre_get_users', array( &$this ) );
            ...
            $user_table = $wpdb->getPrefix() . 'users';
            if (is_array($qv['fields'])) {
                $qv['fields'] = array_unique($qv['fields']);

                $this->query_fields = array();
                foreach ($qv['fields'] as $field) {
                    $field = 'uid' === $field ? 'uid' : sanitize_key($field);
                    $this->query_fields[] = "$user_table.$field";
                }
                $this->query_fields = implode(',', $this->query_fields);
            } elseif ('all' === $qv['fields']) {
                $this->query_fields = "$user_table.*";
            } else {
                $this->query_fields = "$user_table.uid";
            }
            ...
            $this->query_from = "FROM $user_table";
            ...
            if ($qv['has_published_posts'] && $blog_id) {
                ...
                $this->query_where .= " AND $user_table.uid IN ( SELECT DISTINCT $posts_table.post_author FROM $posts_table WHERE $posts_table.post_status = 'publish' AND $posts_table.post_type IN ( " . implode(', ', $post_types) . ' ) )';
            }

            // nicename
            if ('' !== $qv['nicename']) {
                $this->query_where .= $wpdb->prepare(' AND name = %s', $qv['nicename']);
            }

            if (!empty($qv['nicename__in'])) {
                $sanitized_nicename__in = array_map('esc_sql', $qv['nicename__in']);
                $nicename__in = implode("','", $sanitized_nicename__in);
                $this->query_where .= " AND name IN ( '$nicename__in' )";
            }
            ...
            if (!empty($qv['nicename__not_in'])) {
                ...
                $this->query_where .= " AND name NOT IN ( '$nicename__not_in' )";
            }
...
//        }
            ...
            if (!empty($this->meta_query->queries)) {
                $clauses = $this->meta_query->get_sql('user', $user_table, 'uid', $this);
            ...
            if ($search) {
                ...
                if ($qv['search_columns']) {
                    $search_columns = array_intersect($qv['search_columns'], array('uid', 'mail', 'url', 'name', 'screenName'));
                }
                if (!$search_columns) {
                    if (false !== strpos($search, '@')) {
                        $search_columns = array('mail');
                    } elseif (is_numeric($search)) {
                        $search_columns = array('uid');
                    } elseif (preg_match('|^https?://|', $search) && !(is_multisite() && wp_is_large_network('users'))) {
                        $search_columns = array('url');
                    } else {
                        $search_columns = array('url', 'mail', 'name', 'screenName');
                    }
                }
                 ...
                 *
                 * The default columns depend on the search term, and include 'ID',
                 * 'mail', 'url', 'name', and 'screenName'.
                 *
            ...
            if (!empty($include)) {
                // Sanitized earlier.
                $ids = implode(',', $include);
                $this->query_where .= " AND $user_table.uid IN ($ids)";
            } elseif (!empty($qv['exclude'])) {
                $ids = implode(',', wp_parse_id_list($qv['exclude']));
                $this->query_where .= " AND $user_table.uid NOT IN ($ids)";
            }
        ...
        public function query()
        {
            ...
            if (null === $this->results) {
                ...
                if (is_array($qv['fields']) || 'all' === $qv['fields']) {
                    $this->results = $wpdb->fetchAll($this->request);
                } else {
                    $this->results = $wpdb->fetchRow($this->request);
                }
                ...
                if (isset($qv['count_total']) && $qv['count_total']) {
                    ...
                    $this->total_users = (int)$wpdb->fetchRow($found_users_query) ["FOUND_ROWS()"];
                }
            }
            ...
            $this->results = array_map('convert_to_object', $this->results);
...
//        }
        ...
        protected function parse_orderby($orderby)
        {
            ...
            $_orderby = '';
            if (in_array($orderby, array('uid', 'nicename', 'email', 'url', 'registered'), true)) {
                $_orderby = $orderby;
            } elseif (in_array($orderby, array('name', 'mail', 'url', 'user_registered'), true)) {
                $_orderby = $orderby;
            } elseif ('name' === $orderby || 'screenName' === $orderby) {
                $_orderby = 'screenName';
			...
			) p ON ({$user_table}.uid = p.post_author)
			";
                $_orderby = 'post_count';
            } elseif ('uid' === $orderby || 'id' === $orderby) {
                $_orderby = 'uid';
