...
//        }
            ...
            if ($taxonomies) {
                $this->sql_clauses['where']['taxonomy'] = "t.type IN ('" . implode("', '", array_map('esc_sql', $taxonomies)) . "')";
            }
...
//        }
            ...
            switch ($args['fields']) {
                ...
                case 'ids':
                case 'id=>parent':
                    $selects = array('t.term_id', 'tt.parent', 'tt.count', 'tt.type');
                    break;
                case 'names':
                    $selects = array('t.term_id', 'tt.parent', 'tt.count', 't.name', 'tt.type');
                    break;
                case 'count':
                    $orderby = '';
                    $order = '';
                    ...
                    break;
                case 'id=>name':
                    $selects = array('t.term_id', 't.name', 'tt.parent', 'tt.count', 'tt.type');
                    break;
                case 'id=>slug':
                    $selects = array('t.term_id', 't.slug', 'tt.parent', 'tt.count', 'tt.type');
                    break;
            }
            ...
            // typecho don't need join term_taxonomy
...
//        }
            ...
            $table__ = $wpdb->getPrefix() . 'metas';
            $this->sql_clauses['select'] = "SELECT $distinct $fields";
            $this->sql_clauses['from'] = "FROM $table__ AS t $join";
            ...
            $key = md5(serialize(wp_array_slice_assoc($args, array_keys($this->query_var_defaults))) . $this->request);
...
//            }
            ...
            if ('count' === $_fields) {
                $count = $wpdb->fetchRow($this->request)['count'];
                wp_cache_set($cache_key, $count, 'terms');
                return $count;
            }

            $terms_ = $wpdb->fetchAll($this->request);
            $terms = [];
            foreach ($terms_ as $cat) {
                $tmpcat = (object)$cat;
                $terms [] = $tmpcat;
            }
...
//        }
        ...
        protected function get_search_sql($string)
        {
            ...
            $like = '%' . esc_like($string) . '%';

            return sprintf("((t.name LIKE '%s') OR (t.slug LIKE '%s'))", $like, $like);
        }
