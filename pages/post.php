...
<div class="post-meta"><i class="time outline icon"></i><time datetime="<?php $this->date('c'); ?>" itemprop="datePublished"><?php $this->date(); ?></time><span> | </span><i class="folder open outline icon"></i><span class="category"><?php $this->category(','); ?></span><?php if($this->fields->Hot == '1'): ?> | <span><i class="hotjar icon" data-scid="<?php echo $this->cid; ?>"></i>热度:<hot id="ahot"></hot>°C</span><?php endif; ?> | <button class="ui mini gray icon button" id="fontsizes"><i class="font icon"></i></button><?php if(Bsoptions('Readmode') == true): ?><button class="ui mini gray icon button" id="read"><i class="book icon"></i></button><?php endif; ?><write id="editbtn" style="display:none;">|  <button onclick="window.open('<?php $this->options->adminUrl('/write-post.php?cid='.$this->cid); ?>','_self')" class="ui mini gray icon button"><i class="pencil icon"></i></button></write><?php if(Bsoptions('Poster') == true && $this->fields->Poster == '1'): ?>| <button class="ui mini gray icon button bear-btn-bwqr bear-share-poster j-bwqr-poster-btn" data-cid="<?php echo $this->cid;?>" id="poster-btn">生成微海报</button><?php endif; ?></div>
...
<div class="ui basic tag label">标签：<?php $this->tags('  ', true, '暂无标签'); ?></div>
 <?php endif;?>
...
window.poster_theme=<?php if(Bsoptions('Poster__Skin') == ''):?>0<?php else:?><?php echo Bsoptions('Poster__Skin');?><?php endif; ?>;
</script>
<?php endif; ?>
