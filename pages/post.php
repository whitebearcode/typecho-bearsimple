...
<div class="post-meta"><time datetime="<?php $this->date('c'); ?>" itemprop="datePublished"><?php $this->date(); ?></time><span> | </span><span class="category"><?php $this->category(','); ?></span><?php if($this->fields->Hot == '1'): ?> | <i class="hotjar icon"></i>热度:<?php _e(getViewsStr($this));?>°C</span><?php endif; ?> | <button class="ui mini gray icon button" id="fontsizechange"><i class="font icon"></i></button><?php if($this->user->group == 'administrator'): ?>|  <button onclick="window.open('<?php $this->options->adminUrl('/write-post.php?cid='.$this->cid); ?>','_self')" class="ui mini gray icon button"><i class="pencil icon"></i></button><?php endif; ?><?php if($this->options->Poster == '1' && $this->fields->Poster == '1'): ?>| <button href="#" onclick="show_bearstudio_poster_ykzn();return false;" class="ui mini gray icon button">生成微海报</button><?php endif; ?> </div>
