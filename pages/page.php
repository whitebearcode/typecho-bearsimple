...
<div class="post-meta"><i class="user icon"></i><?php $this->author(); ?> <i class="time outline icon"></i><time datetime="<?php $this->date('c'); ?>" itemprop="datePublished"><?php $this->date(); ?></time> <?php if($this->fields->Hot == '1'): ?>  <span><i class="hotjar icon" data-scid="<?php echo $this->cid; ?>"></i>热度:<hot id="ahot"></hot>°C</span><?php endif; ?>  <span id="fontsizes"><i class="font icon"></i></span> <?php if(Bsoptions('Readmode') == true): ?><span id="read"><i class="book reader icon"></i></span><?php endif; ?> <write id="editbtn" style="display:none;">  <span onclick="window.open('<?php $this->options->adminUrl('/write-page.php?cid='.$this->cid); ?>','_self')"><i class="pencil icon"></i></span></write><?php if(Bsoptions('Poster') == true && $this->fields->Poster == '1'): ?> <span class="bear-btn-bwqr bear-share-poster j-bwqr-poster-btn" data-cid="<?php echo $this->cid;?>" id="poster-btn"><i class="newspaper icon"></i></span><?php endif; ?></div>
